package com.ordinary.trifle.common.security.service;
 ...
 * @date 2021/12/21
 */
public interface TrifleUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new TrifleUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	...
	/**
	 * 通过用户实体查询
	 * @param trifleUser user
	 * @return
	 */
	default UserDetails loadUserByUser(TrifleUser trifleUser) {
		return this.loadUserByUsername(trifleUser.getUsername());
	}
